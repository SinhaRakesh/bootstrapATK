    ...
    function page_bhavexecute(){
        ...
        if($_FILES['csv_bhav_file']){
            if ( $_FILES["csv_bhav_file"]["error"] > 0 ) {
                $this->add( 'View_Error' )->set( "Error: " . $_FILES["csv_bhav_file"]["error"] ." csv file not found");
                $this->add('Button')->set("return to importer")->js('click')->univ()->redirect($this->app->url('import'));
            }else{
                ...
                $ret = $company->updateDailyBhav($data);

                $col = $this->add('Columns');
                $col1 = $col->addColumn(6);
                $col2 = $col->addColumn(6);

                $col1->add('View_Info')->set('Record to be import = '.count($data));
                $col2->add('View_Success')->set('Total Records Imported = '.$ret);
                $_FILES['csv_bhav_file']['tmp_name'] = "";
                $_FILES['csv_bhav_file'] = "";
                $this->add('Button')->set("return to importer")->js('click')->univ()->redirect($this->app->url('import'));
            }
        }
    }
