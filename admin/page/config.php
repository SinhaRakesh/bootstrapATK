...
class page_config extends Page {
    ...
    function init() {
        ...
        // $crud = $this->add('CRUD');
        // $crud->setModel('Model_Transaction');

        $crud = $user_tab->add('CRUD');
        $crud->setModel('User');
        
        if($_GET['import_state']){
            // $array = [
            ...
            foreach ($array as $state_name=>$city) {
                $state = $this->add('Model_State')
                    ->addCondition('name',$state_name)
                    ;
                $state->tryLoadAny()->save();

                $state_id = $state->id;

                if(is_array($city)){
                    foreach ($city as $key => $c_name) {
                        $city_m = $this->add('Model_City')
                            ->addCondition('state_id',$state_id)
                            ->addCondition('name',$c_name)
